  ...
  <div>
    <div class="not-started" v-if="!started">
      <a-row :gutter="10" style="padding-top: 40px">
        ...
        <a-col :lg="12" :sm="32" :xs="32" class="leader-wrap">
          <div class="bordered">
            <p class="login-header" style="font-size: 20px; border-bottom: 1px solid #eee; padding-bottom: 12px;">
              Start Examination
            </p>

            <label>Select Subject</label>
            <select name="" id="" v-model="exam.subject" style="margin-bottom: 20px">
              <option v-for="subject in Object.values(subjects)" :value="subject" :key="subject">{{ subject }}</option>
            </select>

            <label>Duration</label>
            <a-input id="password" v-model="exam.duration" label="text" style="margin-top: 0px" />
            <a-button type="primary" :loading="loading" style="width; 100%; height: 40px; margin-top: 20px" @click="proceed"
              >Start</a-button
            >
          </div>
        </a-col>
    ...
    <div class="started" v-else>
      <span style="font-size: 20px; font-weight: 600; text-transform: capitalize">
        {{ info.subject }}
      </span>
      <flip-countdown style="margin-bottom: 20px; float: right" :deadline="newTime" />
      <a-steps :current="current" size="small">
        <a-step v-for="item in questions" :key="item.id" />
      </a-steps>
      <div class="steps-content">
        <question :question="questions[current]" :number="current + 1" @answered="manageScores" />
      </div>
      <div class="steps-action">
        <a-button v-if="current > 0" @click="prev">
          Previous
        </a-button>
        <a-button v-if="current < questions.length - 1" style="margin-left: 8px" type="primary" @click="next">
          Next
        </a-button>
        <a-button
